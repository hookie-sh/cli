...
7. **Tag and push (only after confirmation)**
   ...
   - After a successful push, remind the user that the **Release CLI** workflow will run and that `CLERK_PUBLISHABLE_KEY` and `NPM_TOKEN` must be set in repo secrets. The release notes (changelog) are generated by GoReleaser from conventional commits and appear on the GitHub release page.

## Retrying a failed release

The workflow is idempotent on the same tag:

- **Re-run failed jobs** on a failed workflow run: skips GoReleaser if the GitHub release already exists; retries npm publish.
...
- The release workflow publishes **`@hookie-sh/cli`** to npm. Postinstall downloads binaries from `https://github.com/hookie-sh/cli/releases/download/...`.
