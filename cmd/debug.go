...
// Version is set at build time via ldflags (e.g. goreleaser: -X ...Version={{ .Version }}).
var Version string
...
func printDebugInfo(commandName, orgID, fullCommand string) {
	...
	if configDirEnv := os.Getenv("HOOKIE_CONFIG_DIR"); configDirEnv != "" {
		fmt.Printf("  HOOKIE_CONFIG_DIR: %s\n", configDirEnv)
	} else {
		fmt.Println("  HOOKIE_CONFIG_DIR: (not set)")
	}
	if relayURLEnv != "" {
...
// determineTLSMode reports TLS vs plaintext to match relay client logic.
func determineTLSMode(relayURL string) string {
	if os.Getenv("HOOKIE_INSECURE_TLS") != "" {
		return "insecure (HOOKIE_INSECURE_TLS set)"
	}
	if isLocalhost(relayURL) {
		return "insecure (localhost)"
	}
	host, port := hostPortFromURL(relayURL)
	if port == "443" {
		return "secure (port 443)"
	}
	defaultHost, _ := hostPortFromURL(relay.GetRelayURL())
	if host != "" && host == defaultHost {
		return "secure (production host, TCP Proxy)"
	}
	return "insecure (port != 443)"
}

func hostPortFromURL(url string) (host, port string) {
...
// VersionString returns the CLI version for display (e.g. hookie -v).
