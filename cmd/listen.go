...
// Parameters:
//   - sourceID: Source ID to listen to (empty for app level)
//   - appID: Application ID to listen to (empty for source level)
//   - orgID: Organization ID (used for access verification)
//   - endpointURL: Optional default URL to forward events to
//   - sourceForwardMap: Optional map of source_id -> forward URL for per-source forwarding
//   - guiURL: Optional URL of local GUI server to ingest events into
func runListen(sourceID, appID, orgID string, endpointURL *url.URL, sourceForwardMap map[string]*url.URL, guiURL *url.URL) error {
	cfg, err := config.Load()
	if err != nil {
	...
	stream, err := client.Subscribe(ctx, appID, sourceID, orgID, cfg.MachineID)
	if err != nil {
	...
	if sourceID != "" {
		subscriptionInfo = fmt.Sprintf("source: %s", color.CyanString(sourceID))
	} else if appID != "" {
		subscriptionInfo = fmt.Sprintf("app: %s, all sources", color.CyanString(appID))
	}
	if endpointURL != nil {
	...
	for {
		...
		// Debug logging
		if debug {
			fmt.Fprintf(os.Stderr, "[DEBUG] Received event from gRPC: appID=%s, sourceID=%s, method=%s, path=%s\n",
				event.AppId, event.SourceId, event.Method, event.Path)
		}
		...
		// Priority: source-specific URL > default URL
		var forwardURL *url.URL
		if sourceForwardMap != nil && event.SourceId != "" {
			if sourceURL, exists := sourceForwardMap[event.SourceId]; exists {
				forwardURL = sourceURL
			}
		}
		// Fall back to default forward URL if no source-specific URL
		if forwardURL == nil {
		...
		totalEventsReceived++
		if debug {
			fmt.Fprintf(os.Stderr, "[DEBUG] Total events received: %d, Processing event ID=%d: appID=%s, sourceID=%s\n",
				totalEventsReceived, eventID, event.AppId, event.SourceId)
		}
		...
		if forwardURL != nil {
			go forwardEvent(httpClient, event, forwardURL, eventID, event.AppId, event.SourceId)
		}
...
func formatForwardingMessage(appID, sourceID, action, errorMsg string, eventID uint64, isError bool) string {
	var prefix string
	if isError {
	...
	// Match the event header format: [app_id] with optional (source: source_id)
	context := fmt.Sprintf("[%s]", color.GreenString(appID))
	if sourceID != "" {
		context += fmt.Sprintf(" (source: %s)", color.CyanString(sourceID))
	}
...
func printEvent(event *proto.Event, debug bool) {
	...
	if event.SourceId != "" {
		fmt.Printf(" (source: %s)", color.CyanString(event.SourceId))
	}
...
func forwardEvent(client *http.Client, event *proto.Event, baseURL *url.URL, eventID uint64, appID, sourceID string) {
	...
	if err != nil {
		...
		msg := formatForwardingMessage(appID, sourceID, "failed to create request", err.Error(), eventID, true)
		select {
	...
	if err != nil {
		// Queue error message for sequential printing with event context
		msg := formatForwardingMessage(appID, sourceID, fmt.Sprintf("failed to forward to %s", forwardURL.String()), err.Error(), eventID, true)
		select {
		case forwardingResults <- forwardingResult{eventID: eventID, message: msg}:
		default:
			// Channel full, fall back to direct logging
	...
	msg := formatForwardingMessage(appID, sourceID, fmt.Sprintf("forwarded to %s (status: %d)", forwardURL.String(), resp.StatusCode), "", eventID, false)
	select {
	case forwardingResults <- forwardingResult{eventID: eventID, message: msg}:
	default:
		// Channel full, fall back to direct logging
