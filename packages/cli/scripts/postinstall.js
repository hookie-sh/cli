...
if (!version || version === "0.0.0") {
  console.warn("@hookie-sh/cli: skipping postinstall (version 0.0.0 or not set)");
  process.exit(0);
}
...
if (!goOs || !goArch) {
  console.error(`@hookie-sh/cli: unsupported platform ${platform}/${arch}`);
  process.exit(1);
}

const ext = platform === "win32" ? "zip" : "tar.gz";
const artifactName = `hookie_${version}_${goOs}_${goArch}.${ext}`;
const url = `https://github.com/hookie-sh/cli/releases/download/${tag}/${artifactName}`;
...
async function main() {
  try {
    ...
    console.error("@hookie-sh/cli postinstall failed:", err.message);
    process.exit(1);
  }
}
